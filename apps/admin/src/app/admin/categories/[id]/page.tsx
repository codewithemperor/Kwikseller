...
export default function EditCategoryPage() {
  ...
  useEffect(() => {
    if (category) { const c = category as unknown as Record<string, unknown>; reset({ name: c.name as string, description: (c.description as string) ?? undefined, parentId: (c.parentId as string) ?? undefined, isActive: (c.isActive as boolean) ?? true }); setImageUrl((c.imageUrl as string) ?? ""); }
  }, [category, reset]);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CategoryFormData) => categoriesApi.update(id, { ...data, imageUrl: imageUrl || undefined }),
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <form onSubmit={handleSubmit((d) => updateMutation.mutate(d))} className="space-y-6">
        <div className="rounded-xl border border-default-200 bg-background p-4 lg:p-6 space-y-4">
          ...
          <div><label className={l}>Category Image</label><ImageUpload images={imageUrl ? [imageUrl] : []} onChange={(urls) => setImageUrl(urls[0] ?? "")} maxImages={1} /></div>
