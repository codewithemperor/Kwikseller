/**
 * HTTP client adapter (DEPRECATED — thin shim over @/lib/api-client).
 *
 * WHY THIS EXISTS
 * ──────────────
 * Historically `@/lib/utils` shipped its OWN axios instance with its own
 * refresh-token queue. Running it alongside `@/lib/api-client` meant TWO
