/**
 * KWIKSELLER Admin Panel - Middleware / Proxy
 *
 * Responsibilities:
 * 1. Proxy API requests to the NestJS backend on port 4000
 * 2. Handle subdomain routing
 * 3. Add security headers (stricter for admin panel)
 */
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith("/api/")) {
    ...
    try {
      ...
      console.error("[Admin Proxy] API proxy error:", error);
  ...
  if (!isAdminSubdomain) {
    ...
    if (subdomain === "rider") {
      return NextResponse.redirect(`https://rider.${MAIN_DOMAIN}${pathname}`);
    }
    // Default redirect to main marketplace
    return NextResponse.redirect(`https://${MAIN_DOMAIN}${pathname}`);
  }

  // ── 4. Normal request - add security headers (stricter for admin) ───────────
