...
export const vendorCommerceApi = {
  ...
  submitDeliveryQuote: (
    orderId: string,
    data: { amount: number; minDeliveryDays: number; maxDeliveryDays: number; note?: string },
  ) =>
  ...
  completePickup: (orderId: string, data?: { note?: string }) =>
    api.post(`/orders/${orderId}/complete-pickup`, data),
  ...
  prepareOrder: (orderId: string) =>
    api.post(`/orders/${orderId}/prepare`),

  readyOrder: (orderId: string) =>
    api.post(`/orders/${orderId}/ready-for-pickup`),
