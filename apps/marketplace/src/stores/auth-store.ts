...
function syncLegacyTokenStorage(tokens: AuthTokens | null) {
  ...
  localStorage.setItem("kwikseller_access_token", tokens.accessToken);
  localStorage.setItem("kwikseller_refresh_token", tokens.refreshToken ?? "");
}
  ...
  persist(
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (state) {
          syncLegacyTokenStorage(state.tokens);
